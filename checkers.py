"""
   We used the open source checkers implementation on GitHub: https://github.com/dimitrijekaranfilovic/checkers
   We adopted it to meet our needs. Details can be seen in report.
"""
...
class Checkers:
    ...
    def get_player_input(self):
        ...
        print("Now, the legal move: ", available_moves)
            ...
            if len(old) != 2 or len(new) != 2:
                ...
                if not old_i.isdigit() or not old_j.isdigit() or not new_i.isdigit() or not new_j.isdigit():
                    ...
                    if move not in available_moves:
                        ...
                        Checkers.make_a_move(self.matrix, int(old_i), int(old_j), int(new_i), int(new_j), "B", 0, True)
                        for m in range(8):
                            for n in range(8):
    ...
    def evaluate_states(self):
        ...
        Checkers.make_a_move(self.matrix, int(move[0]), int(move[1]), int(move[2]), int(move[3]), "B", 0, True)
        t2 = time.time()
        diff = t2 - t1
        print("Computer has moved (" + str(move[0]) + "," + str(move[1]) + ") to (" + str(move[2]) + "," + str(
            move[3]) + ").")
        print("It took him " + str(diff) + " seconds.")
        return (move[0]+1)*1000+(move[1]+1)*100+(move[2]+1)*10+(move[3]+1)*1
    ...
    @staticmethod
    def make_a_move(board, old_i, old_j, new_i, new_j, big_letter, queen_row, real=False):
        letter = board[old_i][old_j][0]
        i_difference = old_i - new_i
        j_difference = old_j - new_j
        if i_difference == -2 and j_difference == 2:
            if board[old_i + 1][old_j - 1] != "---" and real:
                print("Eat piece")
                move = str(old_i + 1 + 1) + str(old_j - 1 + 1) + '00'
                print(move)
                value = write_read(str(move))
                print(value)
